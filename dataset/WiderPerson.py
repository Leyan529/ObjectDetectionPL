...
def visualize(image, bboxes, category_ids, category_id_to_name):
    ...
    cv2.waitKey()
# ----------------------------------------------
class WiderPersonDetection(Dataset):
    export_dir = "dataset/export"
    def __init__(self, split, img_size=416):
        root_path = "D:\\WorkSpace\\JupyterWorkSpace\\DataSet\\Object-Detection\\WiderPerson"
        self.image_path = os.path.join(root_path, "Images")  
        self.data_path = os.path.join(root_path, "Annotations")  
        id_list_path = os.path.join(root_path, "%s.txt" % (split))
        self.ids = [id.strip() for id in open(id_list_path)]

        self.img_files = glob(os.path.join(root_path, "JPEGImages\\*.jpg"))
        self.ann_file = glob(os.path.join(root_path, "Annotations\\*.txt"))
        # ---------
        #  Parse Json to Label
        # Pascal VOC Bounding box :(x-top left, y-top left,x-bottom right, y-bottom right)
        # https://towardsdatascience.com/coco-data-format-for-object-detection-a4c5eaf518c5
        # ---------      
        self.classes = ['pedestrians', 'riders', 'partially-visible persons', 'ignore regions', 'crowd']   
    ...
    def __getitem__(self, index):
        ...
        # ---------
        image_label_path = os.path.join(self.data_path, "{}.jpg.txt".format(_id))
        objects = []
        with open(image_label_path, "r", encoding="utf-8") as f:
            for line in f.readlines():
                ...
                if xmin >= 0 and ymin >= 0 and (xmax-xmin)>=0 and (ymax-ymin)>=0 :
                    objects.append([xmin, ymin, xmax-xmin,  ymax-ymin,  label-1])           
...
class WiderPersonModule(pl.LightningDataModule):
    def __init__(self, bsz, img_size=416):
        super().__init__()
        self.batch_size = bsz
        self.img_size = img_size
        self.name = "WiderPerson"
        self.classes = ['pedestrians', 'riders', 'partially-visible persons', 'ignore regions', 'crowd'] 

    def setup(self, stage):
        if stage == 'fit' or stage is None:
            train_set = WiderPersonDetection("train", self.img_size)
            val_set = WiderPersonDetection("val", self.img_size)
        ...
        if stage == 'test' or stage is None:
            self.test_dataset = WiderPersonDetection("val", self.img_size)
            ...
            self.pred_dataset = WiderPersonDetection("test", self.img_size)
            self.pred_dataset = DatasetFromSubset(
                self.pred_dataset, 
                transform = A.Compose([
                    A.Resize(self.img_size, self.img_size),
                    # A.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), max_pixel_value=255)
