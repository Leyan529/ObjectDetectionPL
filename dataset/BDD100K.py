...
class BDD100KDetection(Dataset):
    ...
    def __init__(self, split, img_size=416):
        root_path = "D:\\WorkSpace\\JupyterWorkSpace\\DataSet\\Object-Detection\\bdd100k"
        self.classes = ["bike", "bus","car","motor","person","rider","traffic light","traffic sign","train","truck"]   

        self.img_folders = os.listdir(os.path.join(root_path, "images", "track", split))
        self.anno_groups = glob(os.path.join(root_path, "labels", "box_track_20", split, "*.json") ) 

        self.anno = {}
        for folder in self.img_folders:
            # images = glob(os.path.join(root_path, "images", "track", split, folder, "*.jpg"))
            anno_group = os.path.join(root_path, "labels", "box_track_20", split, "%s.json"%(folder))
            f = open(anno_group)
            data = json.load(f)
            ...
            for item in data:
                name = item["name"]                
                img_path = os.path.join(root_path, "images", "track", split, folder, "%s"%(name))
                labels = item["labels"]
                objects = []
                for label in labels:
                    ...
                    # ---------
                    box2d = label["box2d"]
                    xmin, xmax, ymin, ymax = box2d["x1"], box2d["x2"], box2d["y1"], box2d["y2"]
                    ...
                    if xmin >= 0 and ymin >= 0 and (xmax-xmin)>=0 and (ymax-ymin)>=0 :
                        objects.append([xmin, ymin, xmax-xmin,  ymax-ymin,  self.classes.index(category)]) 
                
                name = name.replace(".jpg","")
                if len(objects) > 0 :
                    data_dict = {"img_path":img_path, "objects":objects}
                    self.anno[name] = data_dict
                # print(i)
        ...
        self.anno = list(self.anno.items())

        print('label_files', len(self.anno))
        print('img_files',len(self.anno))

    def __len__(self):
        return len(self.anno)    

    def __getitem__(self, index):
        data_dict = self.anno[index][1]
        # ---------
        #  Image
        # ---------
...
class BDD100KModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        if stage == 'fit' or stage is None:
            train_set = BDD100KDetection("train", self.img_size)
            val_set = BDD100KDetection("val", self.img_size)
        ...
        if stage == 'test' or stage is None:
            self.test_dataset = BDD100KDetection("val", self.img_size)
