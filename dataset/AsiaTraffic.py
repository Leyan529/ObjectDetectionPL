...
def visualize(image, bboxes, category_ids, category_id_to_name):
    ...
    cv2.waitKey()
# ----------------------------------------------
class AssiaDetection(Dataset):
    name = "AsiaTrafficDataset"
...
class AsiaModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        if stage == 'fit' or stage is None:
            full_dataset = AssiaDetection(self.img_size, is_training = True)
        ...
        if stage == 'test' or stage is None:
            self.test_dataset = AssiaDetection(self.img_size, is_training = False)
